...
class CaculateTLoss(nn.Module):
    ...
    def forward(self,pred,gt):
        loss3 = self.loss_section(pred['section_out'], gt['section'])
        loss_tru = self.trunk_crition(pred['trunk_out'], gt['label'])
        if(self.auxiliary_criterion is not None):
            loss_aux=self.auxiliary_criterion(pred['auxiliary_out'],gt['label'])
            loss = (loss_tru + loss_aux+loss3).mean()
        else:
            loss = (loss_tru + loss3).mean()
        return loss
