...
class Attention(nn.Module):
    def __init__(self, num_heads,hidden_size, vis):
...
class Embeddings(nn.Module):
    ...
    """
    def __init__(self, img_size, patch_size,hidden_size,in_channels=2048):
...
class Block(nn.Module):
    def __init__(self, num_heads,hidden_size, vis):
        ...
        self.ffn = Mlp(hidden_size,mlp_dim=4096)
        self.attn = Attention(num_heads,hidden_size, vis)
...
class Encoder(nn.Module):
    def __init__(self, num_heads,num_layers,hidden_size, vis):
        super(Encoder, self).__init__()
        self.vis = vis
        self.layer = nn.ModuleList()
        self.encoder_norm = LayerNorm(hidden_size, eps=1e-6)
        for _ in range(num_layers):
            layer = Block(num_heads,hidden_size,vis)
...
class Transformer(nn.Module):
    def __init__(self, img_size, patch_size, in_channels,out_channels,num_heads,hidden_size,num_layers,vis):
        super(Transformer, self).__init__()
        self.embeddings = Embeddings(img_size=img_size, patch_size=patch_size,hidden_size=hidden_size,in_channels=in_channels )
        self.encoder = Encoder(num_heads,num_layers,hidden_size, vis)
        self.decoder = DecoderCup(hidden_size,out_channels)
